...
class PrithviViT(nn.Module):
    ...
    def forward_features_multilevel(
        self,
        x: torch.Tensor,
        temporal_coords: None | torch.Tensor = None,
        location_coords: None | torch.Tensor = None,
        ...
        """Extract features from multiple intermediate blocks.

        Args:
            feature_indices: 0-based block indices to extract from.
                Defaults to [5, 11, 17, 23] for the 24-block (300M) model.

        Returns:
            List of tensors, each (B, 1+T*H*W, embed_dim), one per index.
            Raw block outputs (no final LayerNorm applied) so the downstream
            fusion module can apply its own per-level normalization.
        """
        if feature_indices is None:
            step = len(self.blocks) // 4
            feature_indices = [step * (i + 1) - 1 for i in range(4)]
...
class PrithviMAE(nn.Module):
    ...
    def forward(
        ...
        latent, mask, ids_restore = self.encoder(pixel_values, temporal_coords, location_coords, mask_ratio, noise)
    ...
    def forward_features_multilevel(
        ...
        return self.encoder.forward_features_multilevel(
            x, temporal_coords, location_coords, feature_indices
        )
