...
def get_layer_lr_groups_multiscale(model, head_lr, backbone_lr, layer_decay=0.75):
	"""Build per-layer param groups for multi-scale model.

	Same as get_layer_lr_groups but handles model.fusion instead of model.reshaper.
	"""
	...
	# 2) Fusion parameters — full learning rate (newly initialized)
	fusion_params = [p for p in model.fusion.parameters() if id(p) not in seen_params]
	if fusion_params:
		param_groups.append({
		...
		seen_params.update(id(p) for p in fusion_params)
...
def main():
	...
	# Multi-scale feature indices
	parser.add_argument("--feature_indices", type=int, nargs='+',
	                   default=[5, 11, 17, 23],
	                   help="0-based block indices to extract features from (e.g., 5 11 17 23)")
	...
	feat_str = "-".join(str(i) for i in args.feature_indices)
	wandb_config = {
	...
	print(f"Multi-scale feature indices: {args.feature_indices}")
	...
	if args.load_checkpoint:
		param_groups = get_layer_lr_groups_multiscale(
			model,
			head_lr=head_lr,
